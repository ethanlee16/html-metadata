 ...
 */
exports.parseBase = BBPromise.method(function(chtml, tags, reason, getProperty, getContent, token) {
	var meta = {};
	var subProperty = {
		image : 'url',
		video : 'url',
	...
	metaTags.each(function() {
		...
		if (token) {
			var propList = property.split(token);
			property = propList.pop();
			node = propList.reduce((prop, next, i) => {
				if (typeof prop[next] === 'string') {
					prop[next] = {
						[subProperty[propList[propList.length - 1]] || 'content']: prop[next]
					};
				}

				// If the current node is not present, create it
				if (!prop[next]) {
					prop[next] = {};
				}
				return prop[next];
		...
		if (node[property]) {
			var existing = node[property] instanceof Array 
				? node[property] : [node[property]];
			return node[property] = [...existing, content];
		}
		return node[property] = content;
...
exports.parseOpenGraph = BBPromise.method(function(chtml){
	return exports.parseBase(
		...
		'No OpenGraph found',
		function(element) {
			var prop = element.attr('property');
			if (!prop) return;
...
exports.parseTwitter = BBPromise.method(function(chtml) {
	...
	return exports.parseBase(
		...
		function(element) {
			...
			if (!name || name.indexOf('twitter') === -1) return;	
